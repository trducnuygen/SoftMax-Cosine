...
p = argparse.ArgumentParser(description="SoftMax-Cosine Rescaling pipeline for Places365")
...
def run_classifiers(args, pruned_map):
    ...
    for backbone in BACKBONES:
        for rate in args.rates:
            tag              = f"{rate}-places365"
            model_name       = f"{backbone}_SoftMax_r-{rate}_bins-10"
            ...
            if pruned_map is not None:
                ...
                base_train = datasets.Places365(
                    root=args.data, split="train-standard",
                    small=True, download=False, transform=train_tf)
                print("Example of original train paths:", [s[0] for s in base_train.imgs[:5]])

                base_train.imgs    = [s for s in base_train.imgs    if s[0] in selected_train_abs]
                base_train.samples = base_train.imgs
                print(f"[Step 3] Pruned train size: {len(base_train)}")
                train_loader = torch.utils.data.DataLoader(
                    base_train, batch_size=args.batch_size, shuffle=True,
                    num_workers=args.workers, pin_memory=False)

                base_val = datasets.Places365(
                    root=args.data, split="val", small=True,
                    download=False, transform=val_tf)
                print("Example of original val paths:", [s[0] for s in base_val.imgs[:5]])

                base_val.imgs    = [s for s in base_val.imgs    if s[0] in selected_val_abs]
                base_val.samples = base_val.imgs
                print(f"[Step 3] Pruned val size: {len(base_val)}")
                val_loader = torch.utils.data.DataLoader(
                    base_val, batch_size=args.batch_size, shuffle=False,
                    num_workers=args.workers, pin_memory=False)
            elif pruned_map is None:
                print(f"[Step 3] No pruning requested (rate=1.0). Using full Places365 train/val splits.")
                # If no pruning is requested, use the full Places365 train/val splits
                train_loader = torch.utils.data.DataLoader(
                    datasets.Places365(
                        root=args.data, split="train-standard",
                        small=True, download=False, transform=train_tf),
...
def prune_maps(args):
    ...
    if args.rates[0] == 1.0:
        print(args.rates)
        print("No pruning requested (rate=1.0). Skipping Step 2.")
        return None
    else:
        print(f"Pruning requested at rates: {args.rates}")
        for rate in args.rates:
            output_dir = os.path.join(args.prune_out, f"bins10-r1-{rate}-r2-{rate}-Places365")
